...
## Kinmoon NAS (UGREEN DXP2800) — Storage Pool 1 Status (Updated September 22, 2026)
...
- **Storage capacity status (September 22, 2026):** **73.76% used (1.9TB / 2.6TB, 752GB available)** — capacity crisis substantially resolved this session (was 96% / 118GB free pre-cleanup)
...
## Cloudflare Tunnel & Public Hostname Routing (Updated September 22, 2026)
...
**Total Access applications (September 22, 2026):** 17 applications
...
| cloud.najhin-gaming.com | NextCloud | http://192.168.30.220:80 | HTTP | Path-scoped: `/remote.php`, `/ocs`, `/status.php`, `/public.php` Bypass; all others Allow/Email OTP | ✅ (fixed Sept 22) |
| n8n.najhin-gaming.com | n8n | http://192.168.30.211:5678 | HTTP | Allow — Email OTP | ✅ |
| vault.najhin-gaming.com | vault | http://192.168.30.213:8200 | HTTP | Allow — Email OTP | ✅ |
| passwords.najhin-gaming.com | Vaultwarden | http://192.168.30.214:8080 | HTTP | Allow — Email OTP; `/api` + `/identity` Bypass (native clients) | ✅ (fixed Sept 22) |
| passwords.najhin-gaming.com/api | Vaultwarden API | http://192.168.30.214:8080 | HTTP | Bypass — no auth | ✅ |
| passwords.najhin-gaming.com/identity | Vaultwarden Identity | http://192.168.30.214:8080 | HTTP | Bypass — no auth | ✅ |
| ollama.najhin-gaming.com | ollama webui | http://192.168.30.221:8080 | HTTP | Allow — Email OTP | ✅ |
| grafana.najhin-gaming.com | Grafana | http://192.168.30.203:3000 | HTTP | Allow — Email OTP | ✅ |
| n8n.najhin-gaming.com/webhook | n8n Webhooks | http://192.168.30.211:5678 | HTTP | Bypass — webhook validation required | ✅ |
| ntfy.najhin-gaming.com | ntfy | http://192.168.30.222:2586 | HTTP | Allow — Email OTP | ✅ |
| home.najhin-gaming.com | Pulse | http://192.168.30.208:7655 | HTTP | Allow — Email OTP (muzakkir.kholil06@gmail.com) | ✅ (added Sept 22) |
| cloud.najhin-gaming.com/remote.php | Nextcloud WebDAV | http://192.168.30.220:80 | HTTP | Bypass (app-scoped, created Sept 22) | ✅ |
| cloud.najhin-gaming.com/ocs | Nextcloud OCS | http://192.168.30.220:80 | HTTP | Bypass (app-scoped, created Sept 22) | ✅ |
| cloud.najhin-gaming.com/status.php | Nextcloud Status | http://192.168.30.220:80 | HTTP | Bypass (app-scoped, created Sept 22) | ✅ |
| cloud.najhin-gaming.com/public.php | Nextcloud Public Share | http://192.168.30.220:80 | HTTP | Bypass (app-scoped, created Sept 22) | ✅ |

### DNS Records & Status (Updated September 22, 2026)

**Fixed stale A-records (September 22, 2026):**
- `@` (root) — Updated to 202.184.116.231 (was 202.184.35.79)
- `*` (wildcard) — Updated to 202.184.116.231 (was 202.184.35.79)
- `grafana` — Updated to 202.184.116.231 (was 202.184.35.79)
- `home` — Updated to 202.184.116.231 (was 202.184.35.79)
- `n8n` — Updated to 202.184.116.231 (was 202.184.35.79)
- `panel` — Updated to 202.184.116.231 (was 202.184.35.79)

**Remaining stale game-server records (out of scope, not yet updated):**
- `mc.najhin-gaming.com` — Still on 202.184.35.79 (should be 202.184.116.231)
- `terraria.najhin-gaming.com` — Still on 202.184.35.79 (should be 202.184.116.231)

### Vaultwarden Access Policy Fixes (September 22, 2026)

**Issue:** Bypass-Everyone policy on parent app (passwords.najhin-gaming.com) was silently overriding the Allow/Email-OTP policy per Cloudflare's rule precedence (Bypass evaluates first).

**Result:** Email OTP check never executed; all traffic granted access regardless of authentication status.

**Fix:** Deleted the parent app's Bypass-Everyone policy. Kept `/api` and `/identity` sub-app Bypass policies intact (correct scoping for native clients).

**Current state:** Email OTP now actually enforced on login.

### Nextcloud Access Policy Narrowing (September 22, 2026)

**Previous state:** Whole-domain Bypass-Everyone policy (exposed all of Nextcloud to unauthenticated access).

**Issue:** Excessive scope; WebDAV clients, OCS endpoints, and status checks need bypass, but admin console, file browser, settings should require auth.

**Fix:** Created 4 path-scoped Bypass apps:
- `/remote.php` — WebDAV sync (Obsidian, clients)
- `/ocs` — OCS API (app federation, sharing)
- `/status.php` — Status endpoint (liveness checks)
- `/public.php` — Public share links

**Removed:** Parent app's Bypass-Everyone policy

**Current state:** Admin UI, file browser, settings, and all other paths now require Email OTP.

### Pulse Dashboard Access (September 22, 2026)

**Previous state:** No Cloudflare Access protection — Pulse dashboard was directly accessible without authentication.

**Added:** New Access application `Pulse` with Email OTP policy (muzakkir.kholil06@gmail.com).

**Rationale:** Pulse dashboard displays real-time Proxmox metrics and container status — confidential infrastructure information; needed protection.

### n8n Webhook Bypass Notes (Pending Verification)

**Current state:** n8n Webhooks route (n8n.najhin-gaming.com/webhook) has a Bypass policy for unauthenticated webhook access.

**Pending action:** Manually verify that n8n actually validates a secret header/token on the 5 non-Telegram webhook endpoints (`doc-update`, `doc-sync`, `da-vinci`, `midas-report`, Alertmanager routing) before trusting Cloudflare-bypassed access. Could not be verified this session due to Claude