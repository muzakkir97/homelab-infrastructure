...
|--------|------------|---------------|-------------|
| Router | [ROUTER_IP] | Web UI | Your ISP credentials |
| Switch | [SWITCH_IP] | http://[SWITCH_IP] | admin / [your password] |
| Proxmox | [PROXMOX_IP] | https://[PROXMOX_IP]:8006 | root / [your password] |
| Workstation | [WORKSTATION_IP] | Local | - |

### VLAN Configuration

| VLAN ID | Name | Subnet | Purpose | Current Status |
|---------|------|--------|---------|----------------|
| 1 | Management | 192.168.x.0/24 | Management network | ✅ Active |
...
ping -c 3 [WORKSTATION_IP]  # Workstation
```
...
**Test connectivity:**
```cmd
ping [ROUTER_IP]     # Router
ping [SWITCH_IP]     # Switch
ping [PROXMOX_IP]    # Proxmox
...
| 2 | Proxmox | 1 | VLAN 1 (Untagged), VLAN 10,20,30,40 (Tagged) | Trunk |
| 3 | Work Laptop | 10 | VLAN 10 (Untagged) | Access |
| 4 | Workstation | 1 | VLAN 1 (Untagged) | Access |
...
2. Set PC IP: 192.168.x.x (same subnet as switch)
3. Access: http://[SWITCH_IP]
